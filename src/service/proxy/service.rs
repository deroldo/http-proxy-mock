...
impl ProxyService {
    pub async fn proxy_or_mock(
        ...
        let gateway = create_http_client(&original_url, timeout).await?;
        let headers = header_map_to_vec(headers);
        println!("{:?}", headers);
        let response_result = http_call(context, method, &path, body, gateway, headers).await;

        match response_result {
            Ok(response) => Ok(JsonResponse::new(response.status_code, response.body.unwrap_or(Value::Null), HttpTags::default())),
            Err(error) => Ok(JsonResponse::new(
                error.status_code(),
                error.response_json().unwrap_or(error.response_body().map(Value::String).unwrap_or(Value::Null)),
                HttpTags::default(),
            )),
        }
    }
...
async fn get_mock(
    ...
    if let Ok(feature_name) = get_header("X-MOCK-FEATURE", headers) {
        if let Some(configcat) = context.state().configcat.as_ref() {
            let json_body = if let Some(request_body) = body {
                serde_json::to_string(&request_body).map_err(|error| HttpError::without_body(StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to serialize request body: {error}"), HttpTags::default()))?
            } else {
                "".to_string()
            };
            ...
            if !mock.is_empty() && mock != "None" {
                ...
                let feature =
                    serde_json::from_str::<Feature>(&mock).map_err(|error| HttpError::without_body(StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to deserialize feature {feature_name} with mock: {error}"), HttpTags::default()))?;
                let response_status_code = StatusCode::from_u16(feature.response_status_code).map_err(|error| {
...
fn get_header(
    ...
    headers
        .get(header_name)
        .ok_or(HttpError::without_body(StatusCode::BAD_REQUEST, format!("{header_name} header is missing"), HttpTags::default()))?
        .to_str()
        .map(|value| value.to_string())
        .map_err(|error| HttpError::without_body(StatusCode::BAD_REQUEST, format!("X-FEATURE header is invalid: {error}"), HttpTags::default()))
}

const HEADER_FILTER_LIST: [&str; 4] = ["HOST", "X-MOCK-FEATURE", "X-MOCK-ORIGINAL-URL", "X-MOCK-TIMEOUT"];

fn header_map_to_vec(header_map: &HeaderMap) -> Vec<(&str, &str)> {
    header_map
        .iter()
        .filter(|(key, _)| !HEADER_FILTER_LIST.into_iter().any(|filter| filter == key.as_str().to_uppercase()))
...
async fn create_http_client(
    ...
    HttpClient::new("http-proxy-client", original_url, timeout, 1000)
        .await
        .map_err(|error| HttpError::without_body(StatusCode::BAD_REQUEST, format!("Failed to create proxy gateway: {error}"), HttpTags::default()))
}
...
async fn http_call(
    ...
    gateway: HttpClient,
    headers: Vec<(&str, &str)>,
) -> Result<Response<Value>, HttpError> {
    match method {
        ...
        _ => Err(HttpError::without_body(StatusCode::NOT_IMPLEMENTED, format!("Unsupported method: {method}"), HttpTags::default())),
    }
}
